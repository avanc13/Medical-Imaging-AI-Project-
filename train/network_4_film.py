...
class UNet_FiLM(nn.Module):
    def __init__(self, in_channels, out_channels, te_emb_dim=16):
        ...
        # FiLM
        self.te_emb = nn.Sequential(
            nn.Linear(1, te_emb_dim),
            nn.ReLU(),
            nn.Linear(te_emb_dim, 64 * 2)  # max feature channels for FiLM
        )
    ...
    def forward(self, x, te_vals=None):
        skips = []
        ...
        # process decoder with FiLM
        for i, (layer, skip) in enumerate(zip(self.decoder, reversed(skips))):
            x = self.upsample(x)
            x = torch.cat([x, skip], dim=1)
            
            # Apply FiLM if TE provided
            if te_vals is not None:
                # simple example: sum over echoes
                te_flat = te_vals.view(x.size(0), -1, 1)  # (B, N_echoes, 1)
                film_params = self.te_emb(te_flat)        # (B, N_echoes, 64*2)
                gamma, beta = film_params.chunk(2, dim=-1)
                gamma = gamma.mean(dim=1).view(x.size(0), -1, 1, 1)
                beta  = beta.mean(dim=1).view(x.size(0), -1, 1, 1)
                # clamp channels if decoder has fewer than 64
                C = x.size(1)
                gamma = gamma[:, :C, :, :]
                beta  = beta[:, :C, :, :]
...
model = UNet_FiLM(in_channels=1, out_channels=2).to(device)
